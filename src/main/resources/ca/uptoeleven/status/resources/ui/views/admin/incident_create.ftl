        ...
        <div class="panel panel-default service-checkbox-list">
			<#list services as service>
				<div class="checkbox-inline">
					<label><input type="checkbox" class="service-checkbox" id="service_${service.id}" value="${service.id}">${service.name}</label>
				</div>
			</#list>
        </div>
    </div>
...
<script type="application/javascript">
    $(document).ready(function() {
        $("#submit").click(function() {
			var serviceIds = $("input.service-checkbox:checked").map(function() { return this.value }).get();
            var data = {};
